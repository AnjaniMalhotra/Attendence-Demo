...
def delete_classroom(class_name):
    ...
    if os.path.exists(file_path):
        os.remove(file_path)
    for state in ["attendance_status", "attendance_codes", "attendance_limits", "submitted_rolls"]:
...
def show_admin_panel():
    ...
    for key in ["attendance_status", "attendance_codes", "attendance_limits", "submitted_rolls"]:
    ...
    if st.button("Update Code & Limit"):
        ...
        st.session_state.submitted_rolls[selected_class] = set()  # reset
        save_admin_state()
